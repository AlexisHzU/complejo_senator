...
export const enviar_reservacion = (req, res) => {
    ...
    while (contador < 4) {
        ...
        if ((restaurante == reservacion[contador])) {
            //si la cantidad de personas es mayor que la que soportan los restaurantes pues asi lo devolvemos
            if ((cantidad > max_cantidades[contador])) {
                return res.send(`el restaurante ${restaurante} no pueden ir tantas personas`)
            }
...
export const ver_reservaciones = (req, res) => {
    try {
        ...
        if (TypeError(error)) {
            return res.send('<h2>No hay elementos o ese restaurante no existe, favor volver a /restaurante</h2>')
        }
    };
